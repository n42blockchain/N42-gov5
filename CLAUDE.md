...
- **Build version**: `make n42` / `make build` are reproducible and do not mutate the source tree. Use `make version-bump` (or `make release`, which bumps then builds) explicitly. `VERSION` and `params/version.go` must match; normal builds enforce this with `version-check`.
