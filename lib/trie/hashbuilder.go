...
func (hb *HashBuilder) leaf(keyLength int, keyHex []byte, val rlphacks.RlpSerializable) error {
	if hb.trace {
		fmt.Printf("LEAF %d\n", keyLength)
	}
	if keyLength < 0 {
		return fmt.Errorf("length %d", keyLength)
	}
	if hb.proofElement != nil {
		hb.proofElement.storageKey = copyBytes(keyHex[:len(keyHex)-1])
		hb.proofElement.storageValue = new(uint256.Int).SetBytes(val.RawBytes())
	}
	key := keyHex[len(keyHex)-keyLength:]
	s := &shortNode{Key: copyBytes(key), Val: valueNode(copyBytes(val.RawBytes()))}
	hb.nodeStack = append(hb.nodeStack, s)
	if err := hb.leafHashWithKeyVal(key, val); err != nil {
		return err
	}
	copy(s.ref.data[:], hb.hashStack[len(hb.hashStack)-length.Hash:])
	s.ref.len = hb.hashStack[len(hb.hashStack)-length.Hash-1] - 0x80
	if s.ref.len > 32 {
		s.ref.len = hb.hashStack[len(hb.hashStack)-length.Hash-1] - 0xc0 + 1
		copy(s.ref.data[:], hb.hashStack[len(hb.hashStack)-length.Hash-1:])
	}
	if hb.trace {
...
func (hb *HashBuilder) accountLeaf(keyLength int, keyHex []byte, balance *uint256.Int, nonce uint64, incarnation uint64, fieldSet uint32, accountCodeSize int) (err error) {
	...
	if fieldSet&uint32(4) != 0 {
		...
		if hb.acc.Root != EmptyRoot {
			...
			if root == nil {
				root = hashNode{hash: copyBytes(hb.acc.Root[:])}
			}
		}
		popped++
	}
	...
	if fieldSet&uint32(8) != 0 {
		copy(hb.acc.CodeHash[:], hb.hashStack[len(hb.hashStack)-popped*hashStackStride-length.Hash:len(hb.hashStack)-popped*hashStackStride])
		var ok bool
	...
	if hb.proofElement != nil {
		...
		// case.
		hb.proofElement.storageRootKey = copyBytes(fullKey)
		hb.proofElement.storageRoot = hb.acc.Root
	}
	var accCopy account.StateAccount
	accCopy.Copy(&hb.acc)
	...
	s := &shortNode{Key: copyBytes(key), Val: a}
...
func (hb *HashBuilder) extension(key []byte) error {
	...
	switch n := nd.(type) {
	case nil:
		branchHash := copyBytes(hb.hashStack[len(hb.hashStack)-length.Hash:])
		s = &shortNode{Key: copyBytes(key), Val: hashNode{hash: branchHash}}
	case *fullNode:
		s = &shortNode{Key: copyBytes(key), Val: n}
	default:
...
func (hb *HashBuilder) extensionHash(key []byte) error {
	...
	//capture := types.CopyBytes(branchHash[:length.Hash+1])
	if _, err := writer.Write(branchHash[:length.Hash+1]); err != nil {
		return err
	}
...
func (hb *HashBuilder) branch(set uint16) error {
	...
	for digit := uint(0); digit < 16; digit++ {
		if ((1 << digit) & set) != 0 {
			if nodes[i] == nil {
				f.Children[digit] = hashNode{hash: copyBytes(hashes[hashStackStride*i+1 : hashStackStride*(i+1)])}
			} else {
	...
	copy(f.ref.data[:], hb.hashStack[len(hb.hashStack)-length.Hash:])
	f.ref.len = 32
	if hb.trace {
		fmt.Printf("Stack depth: %d\n", len(hb.nodeStack))
	}
...
func (hb *HashBuilder) RootHash() (types.Hash, error) {
	if !hb.hasRoot() {
		return types.Hash{}, fmt.Errorf("no root in the tree")
	}
	return hb.rootHash(), nil
}

func (hb *HashBuilder) rootHash() types.Hash {
	var hash types.Hash
	top := hb.topHash()
	if len(top) == 33 {
		copy(hash[:], top[1:])
