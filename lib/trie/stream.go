...
type StreamIterator interface {
	Next() (itemType StreamItem, hex1 []byte, aValue *account.StateAccount, hash []byte, value []byte)
}
...
func (it *Iterator) Next() (itemType StreamItem, hex1 []byte, aValue *account.StateAccount, hash []byte, value []byte) {
	for {
		if it.top == 0 {
		...
		case *shortNode:
			...
			case *accountNode:
				...
				return AccountStreamItem, hex, &v.StateAccount, nil, nil
			}
			it.hex = hex
		...
		case *accountNode:
			...
			return AccountStreamItem, hex, &n.StateAccount, nil, nil
		case hashNode:
			if it.trace {
...
func (smi *StreamMergeIterator) Next() (itemType1 StreamItem, hex1 []byte, aValue *account.StateAccount, aCode []byte, hash []byte, value []byte) {
	for {
		if smi.trace {
...
func StreamHash(it *StreamMergeIterator, storagePrefixLen int, hb *HashBuilder, trace bool) (types.Hash, error) {
...
func HashWithModifications(
	t *Trie,
	aKeys types.Hashes, aValues []*account.StateAccount, aCodes [][]byte,
	sKeys []types.Hash, sValues [][]byte,
