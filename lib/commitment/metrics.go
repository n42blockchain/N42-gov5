...
func (m *Metrics) WriteToCSV() {
	...
	if err := writeMetricsToCSV(m.Accounts, m.metricsFilePrefix+"_account.csv"); err != nil {
		panic(err)
	}
...
func (m *Metrics) logMetrics() []any {
	return []any{
		"akeys", fmt.Sprintf("%d", m.addressKeys.Load()), "skeys", fmt.Sprintf("%d", m.storageKeys.Load()),
		"rdb", fmt.Sprintf("%d", m.loadBranch.Load()), "rda", fmt.Sprintf("%d", m.loadAccount.Load()),
		"rds", fmt.Sprintf("%d", m.loadStorage.Load()), "wrb", fmt.Sprintf("%d", m.updateBranch.Load()),
		"cb", fmt.Sprintf("%d", m.cacheBranch.Load()), "ca", fmt.Sprintf("%d", m.cacheAccount.Load()),
		"cs", fmt.Sprintf("%d", m.cacheStorage.Load()),
		"mb", fmt.Sprintf("%d", m.missBranch.Load()), "ma", fmt.Sprintf("%d", m.missAccount.Load()),
		"ms", fmt.Sprintf("%d", m.missStorage.Load()),
		"fld", fmt.Sprintf("%d", m.unfolds.Load()), "pdur", m.spentProcessing.Round(time.Millisecond).String(),
		"fdur", m.spentFolding.Round(time.Millisecond).String(), "ufdur", m.spentUnfolding.Round(time.Millisecond).String(),
	}
}
...
func UnmarshallMetricValuesCsv(filePathPrefix string) ([]MetricValues, error) {
	...
	accMetrics, err := UnmarshallAccountMetricsCsv(filePathPrefix + "_account.csv")
	if err != nil {
		return nil, err
	}
