...
func (rc *RecordingContext) Branch(prefix []byte) ([]byte, kv.Step, error) {
	...
	if data != nil {
		rc.branches[string(types.CopyBytes(prefix))] = types.CopyBytes(data)
	}
...
func (rc *RecordingContext) Account(plainKey []byte) (*Update, error) {
	...
	if u != nil {
		...
		rc.accounts[string(types.CopyBytes(plainKey))] = encoded
	}
	return u, nil
}
...
func (rc *RecordingContext) Storage(plainKey []byte) (*Update, error) {
	...
	if u != nil {
		var numBuf [10]byte
		encoded := u.Encode(nil, numBuf[:])
		rc.storages[string(types.CopyBytes(plainKey))] = encoded
	}
	return u, nil
}

func (rc *RecordingContext) PutBranch(prefix []byte, data []byte, prevData []byte) error {
	rc.putBranches[string(types.CopyBytes(prefix))] = BranchWrite{
		PrevData: types.CopyBytes(prevData),
		NewData:  types.CopyBytes(data),
	}
