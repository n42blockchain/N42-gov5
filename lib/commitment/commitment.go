...
import (
	...
	"github.com/n42blockchain/N42/lib/common/maphash"
	"github.com/n42blockchain/N42/lib/etl"
	"github.com/n42blockchain/N42/lib/kv"
	"github.com/n42blockchain/N42/lib/commitment/internal/metrics"
...
type cellEncodeData struct {
	extension   [64]byte
	accountAddr [20]byte                        // types.Address
	storageAddr [length.Addr + length.Hash]byte // 20 + 32 = 52 bytes
	hash        [32]byte                        // types.Hash
	stateHash   [32]byte                        // types.Hash
...
func encodeDeferredUpdate(
	...
	upd.encoded = types.CopyBytes(update)
	return nil
}
...
func (be *BranchEncoder) CollectUpdate(
	...
	// has to copy :(
	prefixCopy := types.CopyBytes(prefix)
	updateCopy := types.CopyBytes(update)
		...
		if c.storageAddrLen > 0 {
			...
			if c.accountAddrLen > 0 {
				...
				if !bytes.Equal(c.accountAddr[:], c.storageAddr[:length.Addr]) {
					return fmt.Errorf("accountAddr mismatch with storageAddr: %s != %x", types.BytesToAddress(c.accountAddr[:]), types.BytesToHash(c.storageAddr[:length.Addr]))
				}
			} else {
				//nolint:staticcheck
				//fmt.Printf("--- debug --- cell with accountAddrLen=0 and storageAddrLen>0: branchKey=%x, branchKeyLen=%d, uncompactedBranchKey=%x, uncompactedBranchKeyLen=%d, plainKeyNibbles=%x, branchKeyAndExtNibbles=%x, cell=%s\n", branchKey, len(branchKey), uncompactedBranchKey, len(uncompactedBranchKey), plainKeyNibbles, branchKeyAndExtNibbles, c)
...
func (t *Updates) TouchAccount(c *KeyUpdate, val []byte) {
	...
	if acc.CodeHash != c.update.CodeHash {
		if acc.CodeHash == (types.Hash{}) || acc.CodeHash == empty.CodeHash {
		...
		err := t.etl.Load(nil, "", func(k, v []byte, table etl.CurrentTableReader, next etl.LoadNextFunc) error {
			...
			if len(t.batchSlab) >= hashSortBatchSize {
				for i := range t.batchSlab {
					...
					if err := fn(t.batchSlab[i].hashedKey, []byte(t.batchSlab[i].plainKey), nil); err != nil {
						return err
					}
				}
				if warmuper != nil {
		...
		t.tree.Ascend(func(item *KeyUpdate) bool {
			...
			if len(t.batchSlab) >= hashSortBatchSize {
				for i := range t.batchSlab {
					...
					if err := fn(t.batchSlab[i].hashedKey, []byte(t.batchSlab[i].plainKey), t.batchSlab[i].update); err != nil {
						processErr = err
						return false
					}
				}
