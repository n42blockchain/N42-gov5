...
func Test_HexPatriciaHashed_UniqueRepresentation2(t *testing.T) {
	...
	var rSeq, rBatch []byte
	{
		...
		for i := 0; i < len(updates); i++ {
			...
			rSeq = types.CopyBytes(sequentialRoot)
...
func Test_Trie_CorrectSwitchForConcurrentAndSequential(t *testing.T) {
	...
	for i := 0; i < len(plainKeysList); i++ {
		builder.Balance(types.Bytes2Hex(plainKeysList[i]), uint64(i))
	}
	...
	for i := 2; i < len(plainKeys); i++ {
		builder.Delete(types.Bytes2Hex(plainKeys[i]))
	}
	plainKeys, updates = builder.Build()
	err = ms.applyPlainUpdates(plainKeys, updates)
	require.NoError(t, err)
...
func Test_ParallelHexPatriciaHashed_EdgeCases(t *testing.T) {
	...
	for i := 0; i < len(plainKeysList); i++ {
		...
		builder.Balance(types.Bytes2Hex(plainKeysList[i]), 1000*uint64(i))
	}
...
func Test_HexPatriciaHashed_DeferredBranchUpdates(t *testing.T) {
	...
	trieNormal := NewHexPatriciaHashed(length.Addr, stateNormal)
	trieDeferred := NewHexPatriciaHashed(length.Addr, stateDeferred)
	...
	var rSeq, rBatch []byte
	{
		...
		for i := 0; i < len(updates); i++ {
			...
			if i == somewhere {
				...
				require.NoError(t, err)
				somewhereRoot = types.CopyBytes(sequentialRoot)
			}
		}
	}
	{
...
func TestUpdate_EncodeDecode(t *testing.T) {
	...
	updates := []Update{
		...
		{Flags: StorageUpdate, Storage: types.Hash{0x21, 0x22, 0x23, 0x24}, StorageLen: 4, CodeHash: empty.CodeHash},
...
func TestUpdate_Merge(t *testing.T) {
	...
	updates := []tcase{
		...
		},
		{
			...
			b: Update{Flags: BalanceUpdate | NonceUpdate | CodeUpdate, Balance: *uint256.NewInt(45639015), Nonce: 124,
				CodeHash: types.Hash{
					...
					0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17, 0x18,
					0x19, 0x1A, 0x1B, 0x1C, 0x1D, 0x1E, 0x1F, 0x20}},
			e: Update{Flags: BalanceUpdate | NonceUpdate | CodeUpdate, Balance: *uint256.NewInt(45639015), Nonce: 124, CodeHash: types.Hash{
				0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08,
				0x09, 0x0A, 0x0B, 0x0C, 0x0D, 0x0E, 0x0F, 0x10,
				0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17, 0x18,
				0x19, 0x1A, 0x1B, 0x1C, 0x1D, 0x1E, 0x1F, 0x20}},
		...
		{
			a: Update{Flags: StorageUpdate, Storage: types.Hash{0x21, 0x22, 0x23, 0x24}, StorageLen: 4, CodeHash: empty.CodeHash},
		...
		},
		{
			...
			b: Update{Flags: StorageUpdate, Storage: types.Hash{0x21, 0x22, 0x23, 0x24}, StorageLen: 4, CodeHash: empty.CodeHash},
			e: Update{Flags: StorageUpdate, Storage: types.Hash{0x21, 0x22, 0x23, 0x24}, StorageLen: 4, CodeHash: empty.CodeHash},
		},
	}
...
func Test_HexPatriciaHashed_ProcessWithDozensOfStorageKeys(t *testing.T) {
	...
	//		prefixes[string(hashed[:5])] = append(prefixes[string(hashed[:5])], types.CopyBytes(noise))
	//	}
	//}
	//
	//count := 0
...
func Test_WitnessTrie_GenerateWitness(t *testing.T) {
	...
	t.Run("JustRoot", func(t *testing.T) {
		...
		addrWithSingleton := types.CopyBytes(plainKeysList[0])
		builder := NewUpdateBuilder()
		for i := 0; i < len(plainKeysList); i++ {
			builder.Balance(types.Bytes2Hex(plainKeysList[i]), uint64(i))
		}
	...
	t.Run("StorageSingleton", func(t *testing.T) {
		...
		builder.Storage(types.Bytes2Hex(addrWithSingleton), "00044c45500c49b2a2a5dde8dfc7d1e71c894b7b9081866bfd33d5552deed470", "00044c45500c49b2a2a5dde8dfc7d1e71c894b7b9081866bfd33d5552deed470")

		//for sl := 0; sl < len(storageKeysList); sl++ {
		//	builder.Storage(types.Bytes2Hex(addrWithSingleton), types.Bytes2Hex(storageKeysList[sl]), types.Bytes2Hex(storageKeysList[sl]))
		//}
		// fmt.Printf("addrWithSingleton %x\n", addrWithSingleton)
		// builder.Storage(types.Bytes2Hex(addrWithSingleton), "01044c45500c49b2a2a5dde8dfc7d1e71c894b7b9081866bfd33d5552deed470", "00044c45500c49b2a2a5dde8dfc7d1e71c894b7b9081866bfd33d5552deed470")
	...
	t.Run("NonExistentAccountProofShortNodeToAccount", func(t *testing.T) {
		...
		builder.Storage(types.Bytes2Hex(plainKeys52789[0]), "00044c45500c49b2a2a5dde8dfc7d1e71c894b7b9081866bfd33d5552deed470", "00044c45500c49b2a2a5dde8dfc7d1e71c894b7b9081866bfd33d5552deed470")
	...
	t.Run("NonExistentAccountProofShortNodeToFullNode", func(t *testing.T) {
		...
		// add a storage slot to spice up the test case
		builder.Storage(types.Bytes2Hex(plainKeys527a[0]), "00044c45500c49b2a2a5dde8dfc7d1e71c894b7b9081866bfd33d5552deed470", "00044c45500c49b2a2a5dde8dfc7d1e71c894b7b9081866bfd33d5552deed470")
	...
	t.Run("SingletonStorage", func(t *testing.T) {
		...
		addrToProve := types.CopyBytes(plainKeysList[0])
		...
		storageSlotToProve := types.CopyBytes(storagePlainKeysList[0])

		fullStorageKeyToProve := types.CopyBytes(addrToProve)
		...
		for sl := 0; sl < len(storagePlainKeysList); sl++ {
			builder.Storage(types.Bytes2Hex(addrToProve), types.Bytes2Hex(storagePlainKeysList[sl]), types.Bytes2Hex(storagePlainKeysList[sl]))
	...
	t.Run("NonExistentStorageProofShortNodeToValue", func(t *testing.T) {
		...
		for sl := 0; sl < len(storageKeysList); sl++ {
			builder.Storage(types.Bytes2Hex(addrToProve), types.Bytes2Hex(storageKeysList[sl]), types.Bytes2Hex(storageKeysList[sl]))
			fmt.Printf("storage %x -> %x\n", storageKeysList[sl], storageKeysList[sl])
		}
	...
	t.Run("MultiKeyWitness_AccountWithSingletonStorage", func(t *testing.T) {
		...
		addrWithStorage := types.CopyBytes(plainKeysA[0])
		...
		storageSlot := types.CopyBytes(storagePlainKeysList[0])

		// Full storage key = address + storage slot
		fullStorageKey := types.CopyBytes(addrWithStorage)
		...
		builder.Balance(types.Bytes2Hex(plainKeys5[0]), 100)
		fmt.Printf("addr (prefix 0x5) %x\n", plainKeys5[0])

		// Add balance to account with prefix 0xa
		builder.Balance(types.Bytes2Hex(addrWithStorage), 200)
		fmt.Printf("addr (prefix 0xa) %x\n", addrWithStorage)

		// Add singleton storage to 0xa account
		builder.Storage(types.Bytes2Hex(addrWithStorage), types.Bytes2Hex(storageSlot), types.Bytes2Hex(storageSlot))
	...
	t.Run("Multiproof_NonExistentStorageProof", func(t *testing.T) {
		...
		addrWithoutStorage := types.CopyBytes(plainKeys5[len(plainKeys5)-1])

		// Account WITH storage - we'll prove both the account and one of its storage slots
		addrWithStorage := types.CopyBytes(plainKeys5[0])
		...
		fullNonExistentStorageKey := types.CopyBytes(addrWithoutStorage)
		...
		for i, addr := range allAccounts {
			builder.Balance(types.Bytes2Hex(addr), uint64(i+1))
			fmt.Printf("addr %x\n", addr)
			...
			for _, slot := range storageSlots {
				builder.Storage(types.Bytes2Hex(addr), types.Bytes2Hex(slot), types.Bytes2Hex(slot))
			}

			// Save one existing storage slot for the account we'll prove
			if bytes.Equal(addr, addrWithStorage) {
				existingStorageSlot = types.CopyBytes(storageSlots[0])
			}
		}

		// Build full key for the existing storage slot
		fullExistingStorageKey := types.CopyBytes(addrWithStorage)
	...
	t.Run("MultiRound_AccountBalanceUpdate", func(t *testing.T) {
		...
		for i, addr := range accounts {
			builder1.Balance(types.Bytes2Hex(addr), uint64(i+1)*100)
		}

		// Round 2: Update balance of 2 existing accounts
		builder2 := NewUpdateBuilder()
		builder2.Balance(types.Bytes2Hex(accounts[0]), 9999)
		builder2.Balance(types.Bytes2Hex(accounts[2]), 8888)
	...
	t.Run("MultiRound_AddAccountsToExistingTrie", func(t *testing.T) {
		...
		for i, addr := range round1Accounts {
			builder1.Balance(types.Bytes2Hex(addr), uint64(i+1)*10)
		}
		...
		for i, addr := range round2Accounts {
			builder2.Balance(types.Bytes2Hex(addr), uint64(i+1)*20)
		}
	...
	t.Run("MultiRound_StorageUpdateOnExistingAccount", func(t *testing.T) {
		...
		builder1 := NewUpdateBuilder()
		for i, addr := range accounts {
			builder1.Balance(types.Bytes2Hex(addr), uint64(i+1)*50)
		}
		for _, slot := range storageSlots {
			builder1.Storage(types.Bytes2Hex(accounts[0]), types.Bytes2Hex(slot), types.Bytes2Hex(slot))
		}
		...
		builder2.Storage(types.Bytes2Hex(accounts[0]), types.Bytes2Hex(storageSlots[0]), types.Bytes2Hex(updatedValue))
		// Add new slot
		builder2.Storage(types.Bytes2Hex(accounts[0]), types.Bytes2Hex(newSlot), types.Bytes2Hex(newSlot))

		// Witness: updated storage slot + new storage slot
		fullUpdatedKey := append(types.CopyBytes(accounts[0]), storageSlots[0]...)
		fullNewKey := append(types.CopyBytes(accounts[0]), newSlot...)
	...
	t.Run("DeletedAccount_NonExistentProof", func(t *testing.T) {
		...
		builder := NewUpdateBuilder()
		for i, addr := range accounts {
			builder.Balance(types.Bytes2Hex(addr), uint64(i+1)*100)
		}
		// Delete one account in the same round
		builder.Delete(types.Bytes2Hex(accounts[2]))
	...
	t.Run("MultiRound_DeleteAccount_ThenWitness", func(t *testing.T) {
		...
		builder2 := NewUpdateBuilder()
		builder2.Delete(types.Bytes2Hex(accounts[1]))
	...
	t.Run("MultiRound_DeleteStorage_ThenWitness", func(t *testing.T) {
		...
		for i, a := range accounts {
			builder1.Balance(types.Bytes2Hex(a), uint64(i+1)*100)
		}
		for _, slot := range storageSlots {
			builder1.Storage(types.Bytes2Hex(addr), types.Bytes2Hex(slot), types.Bytes2Hex(slot))
		}

		// Round 2: Delete 2 storage slots
		builder2 := NewUpdateBuilder()
		builder2.DeleteStorage(types.Bytes2Hex(addr), types.Bytes2Hex(storageSlots[0]))
		builder2.DeleteStorage(types.Bytes2Hex(addr), types.Bytes2Hex(storageSlots[1]))

		// Witness: deleted slot = false, surviving slot = true
		deletedKey := append(types.CopyBytes(addr), storageSlots[0]...)
		survivingKey := append(types.CopyBytes(addr), storageSlots[3]...)
	...
	t.Run("AccountWithCodeHash", func(t *testing.T) {
		...
		builder.CodeHash(types.Bytes2Hex(accounts[1]), types.Bytes2Hex(codeHashBytes))
	...
	t.Run("AccountWithCodeHash_AndStorage", func(t *testing.T) {
		...
		builder := NewUpdateBuilder()
		for i, a := range accounts {
			builder.Balance(types.Bytes2Hex(a), uint64(i+1)*500)
		}
		builder.CodeHash(types.Bytes2Hex(addr), types.Bytes2Hex(codeHashBytes))
		for _, slot := range storageSlots {
			builder.Storage(types.Bytes2Hex(addr), types.Bytes2Hex(slot), types.Bytes2Hex(slot))
		}

		// Witness: account key + 1 storage slot key
		fullStorageKey := append(types.CopyBytes(addr), storageSlots[1]...)
	...
	t.Run("MixedAccountProof_ExistingAndNonExisting", func(t *testing.T) {
		...
		builder := NewUpdateBuilder()
		for i, addr := range accounts {
			builder.Balance(types.Bytes2Hex(addr), uint64(i+1)*10)
		}
	...
	t.Run("MixedStorageProof_ExistingAndNonExisting", func(t *testing.T) {
		...
		builder := NewUpdateBuilder()
		for i, a := range accounts {
			builder.Balance(types.Bytes2Hex(a), uint64(i+1)*100)
		}
		for _, slot := range storageSlots {
			builder.Storage(types.Bytes2Hex(addr), types.Bytes2Hex(slot), types.Bytes2Hex(slot))
		}
		...
		existKey1 := append(types.CopyBytes(addr), storageSlots[0]...)
		existKey2 := append(types.CopyBytes(addr), storageSlots[3]...)
		nonExistKey1 := append(types.CopyBytes(addr), nonExistSlot1...)
		nonExistKey2 := append(types.CopyBytes(addr), nonExistSlot2...)
	...
	t.Run("DeepStorageTrie_ManySlots", func(t *testing.T) {
		...
		fullStorageKey := append(types.CopyBytes(addr), storageSlots[25]...)
		require.Equal(t, length.Addr+length.Hash, len(fullStorageKey))
	...
	t.Run("AccountWithZeroBalanceNonZeroNonce", func(t *testing.T) {
		...
		builder := NewUpdateBuilder()
		builder.Balance(types.Bytes2Hex(accounts[0]), 0)
		builder.Nonce(types.Bytes2Hex(accounts[0]), 42)
	...
	t.Run("MultipleAccountsWithStorage_WitnessStorageAcrossAccounts", func(t *testing.T) {
		...
		for i, addr := range accounts {
			...
			fullKey := append(types.CopyBytes(addr), storageSlots[1]...)
	...
	t.Run("StorageLeafRLPShorterThan32Bytes", func(t *testing.T) {
		...
		storageSlotToProve := types.CopyBytes(storageKey1)
		...
		builder.Storage(types.Bytes2Hex(addrToProve), types.Bytes2Hex(storageKey1), "01")
		builder.Storage(types.Bytes2Hex(addrToProve), types.Bytes2Hex(storageKey2), "02")
