...
import (
	...
	"github.com/n42blockchain/N42/lib/state/stateifs"
	"github.com/n42blockchain/N42/lib/commitment/trie"
	witnesstypes "github.com/n42blockchain/N42/lib/commitment/witness"
...
type HexPatriciaHashed struct {
	...
	// putBranchFn is called during fold() to persist branch data immediately.
...
type cell struct {
	...
	extension       [64]byte
	accountAddr     types.Address                  // account plain key
	storageAddr     [length.Addr + length.Hash]byte // storage plain key
	hash            types.Hash                     // cell hash
...
func (cell *cell) accountForHashing(buffer []byte, storageRootHash types.Hash) int {
	balanceBytes := 0
...
func (hph *HexPatriciaHashed) extensionHash(key []byte, hash []byte) (types.Hash, error) {
	var hashBuf types.Hash
...
func (hph *HexPatriciaHashed) witnessComputeCellHashWithStorage(cell *cell, depth int16, buf []byte) ([]byte, bool, []byte, error) {
	var err error
	var storageRootHash types.Hash
	...
	if cell.storageAddrLen > 0 {
		...
		if cell.stateHashLen > 0 {
			...
			if !singleton {
				...
				storageRootHash = *(*types.Hash)(res[1:])
			...
			if singleton {
				...
				storageRootHash = *(*types.Hash)(aux[1:])
				storageRootHashIsSet = true
				cell.stateHashLen = 0
				hadToReset.Add(1)
			} else {
...
func (hph *HexPatriciaHashed) computeCellHash(cell *cell, depth int16, buf []byte) ([]byte, error) {
	...
	if cell.storageAddrLen > 0 {
		...
		if cell.stateHashLen > 0 {
			...
			storageRootHashIsSet = true
			storageRootHash = *(*types.Hash)(cell.stateHash[:cell.stateHashLen])
		} else {
			koffset := hph.accountKeyLen
			if depth == 0 && cell.accountAddrLen == 0 {
				// if account key is empty, then we need to hash storage key from the key beginning
			...
			storageRootHash = *(*types.Hash)(leafHash[1:])
			storageRootHashIsSet = true
			cell.stateHashLen = 0
			hadToReset.Add(1)
		}
...
func (hph *HexPatriciaHashed) witnessCreateAccountNode(c *cell, depth int16, hashedKey []byte, codeReads map[types.Hash]witnesstypes.CodeWithHash) (*trie.AccountNode, error) {
	...
	var account account.StateAccount
	...
	if !hasCode {
		...
		// sanity check
		if account.CodeHash != codeWithHash.Hash {
			return nil, fmt.Errorf("account.CodeHash(%x)!=codeReads[%x].CodeHash(%x)", account.CodeHash, addrHash, codeWithHash.Hash)
		}
	}
	...
	if !storageIsSet {
		...
		accountNode = &trie.AccountNode{Account: account, Storage: trie.NewHashNode(types.BytesToHash(storageRootHash)), RootCorrect: true, Code: code, CodeSize: -1}
	}
	return accountNode, nil
}

// Traverse the grid following `hashedKey` and produce the witness `triedeprecated.Trie` for that key
func (hph *HexPatriciaHashed) toWitnessTrie(hashedKey []byte, codeReads map[types.Hash]witnesstypes.CodeWithHash) (*trie.Trie, error) {
	...
	if hph.root.hashedExtLen > 0 {
		extKey := types.CopyBytes(hph.root.hashedExtension[:hph.root.hashedExtLen])
		if len(extKey) == 64 {
	...
	for row := 0; row < hph.activeRows && keyPos < int16(len(hashedKey)); row++ {
		...
		if cellToExpand.hashedExtLen > 0 { // extension cell
			...
			if !bytes.Equal(hashedExtKey, hashedKeySubstring) && fullPathLength != 64 && fullPathLength != 128 {
				...
				nextNode = &trie.ShortNode{Key: types.CopyBytes(hashedExtKey)}
			} else {
				...
				if keyPos+1 == int16(len(hashedKey)) || keyPos+1 == 64 {
					if cellToExpand.storageAddrLen > 0 && !depthAdjusted {
						...
						if false { // TODO: witness verify
							_ = subTrieRoot; return nil, fmt.Errorf("witness verify failed: %x", cellHash)
						}
			...
			if _, ok := currentNode.(*trie.AccountNode); ok && len(hashedKey) > 64 {
				if cellToExpand.hashedExtLen > 0 {
					extKey := types.CopyBytes(cellToExpand.hashedExtension[:cellToExpand.hashedExtLen])
		...
		if fullNode, ok := currentNode.(*trie.FullNode); ok { // handle full node case
			for col := 0; col < 16; col++ {
				...
				fullNode.Children[col] = trie.NewHashNode(types.BytesToHash(cellHash))
			...
			if pathDivergenceFound {
				...
				nextNode.(*trie.ShortNode).Val = trie.NewHashNode(types.BytesToHash(terminalCell.hash[:]))
			}
		...
		if nextShortNode, ok := nextNode.(*trie.ShortNode); ok && len(hashedKey) > 64 && keyPos+1 == 64 {
			...
			if nextAccNode, ok := nextShortNode.Val.(*trie.AccountNode); ok {
				if nextAccNode.Account.Root != trie.EmptyRoot {
...
func (hph *HexPatriciaHashed) foldBranch(row int, nibble, upDepth, depth int16, upCell *cell, updateKey []byte) error {
	...
	if hph.branchEncoder.DeferUpdatesEnabled() {
		...
		if err := hph.feedBranchHashesToKeccak(row, depth, b[:]); err != nil {
			return err
		}
...
func (hph *HexPatriciaHashed) GenerateWitness(ctx context.Context, updates *Updates, codeReads map[types.Hash]witnesstypes.CodeWithHash, logPrefix string) (witnessTrie *trie.Trie, rootHash []byte, err error) {
	var (
		m  runtime.MemStats
		ki uint64
	...
	err = updates.HashSort(ctx, nil, func(hashedKey, plainKey []byte, stateUpdate *Update) error {
		...
		case <-logEvery.C:
			...
			log.Info(fmt.Sprintf("[%s][agg] computing trie", logPrefix),
				"progress", fmt.Sprintf("%s/%s", fmt.Sprintf("%d", ki), fmt.Sprintf("%d", updatesCount)),
				"alloc", uint64(m.Alloc), "sys", uint64(m.Sys))
		default:
		}
...
func (hph *HexPatriciaHashed) Process(ctx context.Context, updates *Updates, logPrefix string, onProgress func(*CommitProgress), warmup WarmupConfig) (rootHash []byte, err error) {
	...
	err = updates.HashSort(ctx, warmuper, func(hashedKey, plainKey []byte, stateUpdate *Update) error {
		...
		case <-logEvery.C:
			if onProgress != nil {
				...
				dbg.ReadMemStats(&m)
				log.Info(fmt.Sprintf("[%s][agg] computing trie", logPrefix),
					append(append([]any{"progress", fmt.Sprintf("%s/%s", fmt.Sprintf("%d", ki), fmt.Sprintf("%d", updatesCount))},
						hph.metrics.logMetrics()...), "alloc", uint64(m.Alloc), "sys", uint64(m.Sys))...)
			}
		default:
		}
	...
	if dbg.KVReadLevelledMetrics {
		...
		log.Debug("commitment finished, counters updated (no reset)",
			//"hadToLoad", fmt.Sprintf("%d", hadToLoad.Load()), "skippedLoad", fmt.Sprintf("%d", skippedLoad.Load()),
			//"hadToReset", fmt.Sprintf("%d", hadToReset.Load()),
			...
			"keys", fmt.Sprintf("%d", ki), "spent", time.Since(start),
		)
		...
		for _, k := range ends {
			v := hph.hadToLoadL[k]
			accs := fmt.Sprintf("load=%s skip=%s (%.1f%%) reset %.1f%%", fmt.Sprintf("%d", v.accLoaded), fmt.Sprintf("%d", v.accSkipped), 100*(float64(v.accSkipped)/float64(v.accLoaded+v.accSkipped)), 100*(float64(v.accReset)/float64(v.accReset+v.accSkipped)))
			stors := fmt.Sprintf("load=%s skip=%s (%.1f%%) reset %.1f%%", fmt.Sprintf("%d", v.storLoaded), fmt.Sprintf("%d", v.storSkipped), 100*(float64(v.storSkipped)/float64(v.storLoaded+v.storSkipped)), 100*(float64(v.storReset)/float64(v.storReset+v.storSkipped)))
			if k == 0 {
...
// keccakState wraps sha3 hash state for compatibility with Erigon's keccak interface.
