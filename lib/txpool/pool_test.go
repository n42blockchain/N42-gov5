...
import (
	...
	"github.com/n42blockchain/N42/lib/crypto/kzg"
...
func TestNonceFromAddress(t *testing.T) {
	...
	pool, err := New(ch, coreDB, cfg, sendersCache, *u256.N1, nil, nil, nil, nil, nil, nil, nil, nil, log.New())
	assert.NoError(err)
...
func TestShanghaiValidateTx(t *testing.T) {
	...
	for name, test := range tests {
		t.Run(name, func(t *testing.T) {
			...
			pool, err := New(ch, coreDB, cfg, cache, *u256.N1, shanghaiTime, nil /* agraBlock */, nil /* cancunTime */, nil, nil, nil, nil, nil, logger)
			asrt.NoError(err)
			ctx := context.Background()
...
func TestSetCodeTxValidationWithLargeAuthorizationValues(t *testing.T) {
	...
	pool, err := New(ch, coreDB, cfg, cache, chainID, common.Big0 /* shanghaiTime */, nil, /* agraBlock */
		common.Big0 /* cancunTime */, common.Big0 /* pragueTime */, nil, nil, nil, nil, logger)
...
func TestBlobTxReplacement(t *testing.T) {
	...
	cfg := txpoolcfg.DefaultConfig
	sendersCache := kvcache.New(kvcache.DefaultCoherentConfig)
	pool, err := New(ch, coreDB, cfg, sendersCache, *u256.N1, common.Big0, nil, common.Big0, nil, nil, nil, nil, nil, log.New())
	assert.NoError(err)
	require.True(pool != nil)
	ctx := context.Background()
	var stateVersionID uint64 = 0
...
func TestDropRemoteAtNoGossip(t *testing.T) {
	...
	txPool, err := New(ch, coreDB, cfg, sendersCache, *u256.N1, big.NewInt(0), big.NewInt(0), nil, nil, nil, nil, nil, nil, logger)
	assert.NoError(err)
