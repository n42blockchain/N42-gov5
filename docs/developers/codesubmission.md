...
This is the product's main code branch, maintaining a stable codebase for external use. Direct work on this master branch is not allowed; instead, work should be done on other specified, independent feature branches (we will discuss this shortly). Not directly committing changes to the master branch is also a common rule in many workflows.

### develop

The development branch is the base branch for any new development. When you start a new feature branch, it will be based on develop. Additionally, this branch also collects all completed features, waiting to be integrated into the master branch.
...
Feature branches are used for the development of new functionalities and features.
...
When you believe the code in the "develop" branch is ready for a mature release version, it means:
1. It includes all new features and necessary fixes
2. It has been thoroughly tested

If both points are satisfied, then it's time to create a new release:
1. Merge the develop branch into master.
2. Tag the current master and issue the corresponding release version.

### hotfix

Often, just a few hours or days after a release version has been deployed, some minor errors may be discovered. In this case, we need to create a hotfix branch:
