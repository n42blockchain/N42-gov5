...
| S10 | 35zzy B1 win2 collapse (17 blocks at 3.5 s, node5 block 13659711 dropped 100,700 of 163,000 candidates as nonceLow): where do the 3.5 s go, and how many stale (mined-not-demoted) candidates does the pool hand the builder per block? Commander's ruling after S7: no third round on the sixteen-generator shape -- two rounds (47.0k, 86.2k) both lost to the eight-generator 35zzt (127.6k); the node-side suspect is pool demotion lagging the chain, which no harness change fixes | logs only (urgent: node logs rotate) | not written (diagnostic) | next | |
