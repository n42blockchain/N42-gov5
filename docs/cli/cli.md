...
The ast node is operated via the CLI by running the `ast` command. To stop it, press `ctrl-c`. You may need to wait a bit as ast down existing p2p connections or other cleanup tasks.

However, ast has more commands than that:
...
GLOBAL OPTIONS:
   ...
   --data.dir value                 data save dir (default: "./ast/")
   ...
   --ipcpath value                  Filename for IPC socket/pipe within the data dir (explicit paths escape it) (default: "ast.ipc")
   ...
   --log.name value                 logger file name and path (default: "ast.log")
