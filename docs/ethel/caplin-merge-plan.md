...
- **下一步:** `cl/sentinel` 核心(sentinel.go/gossip.go/discovery.go)+ `handlers/{handlers,blocks}.go`(块-only)+ `service/{service,start}.go`(实现 `sentinelproto.SentinelServer`)→ 真 `cl/rpc/rpc.go`(替换 rpc_stub,经 SendRequest)→ `block_service`(gossip→OnBlock)→ wiring(service.go Start 起 sentinel+ClStages+GetHead 驱动 EL)→ 对抗 E2E。
