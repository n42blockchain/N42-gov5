...
**改造**:建一个冷层 **blockhash-index**(与 txindex 同构):
- `MPHF(blockHash) → number`,Enums:true + 关 LFP → **~2–4 bit/key**;25M 块 ≈ **~10–12 MB**(对比 ~1GB,**~80–100×**)。
...
1. **机会 2**(零新索引,最快):把 txlookup.Service 冷层也接进 getTransactionReceipt —— 纯 wiring,几行。
2. **机会 1**(最大收益):blockhash-index 冷层(MPHF+self-verify header hash),工具/reader/接线复用 txindex 骨架。HeaderNumber 从 MDBX 卸到 ~10MB 冷段,getBlockByHash 家族(29 处)全部受益。
