...
| perf/block-precheck（bp2，未合并） | 44m41s | 318,062 | 基本块预检：正确但更慢，§5.33 |
| **main（含审计修复，`audit1`）** | **42m08s** | **310,373** | 合并前审计 4 项修复（§5.34），+1.7% 判为漂移 |
| **main（ic6 + pool 开关，`main-20260827b`，08-28 重跑）** | **41m28s** | **305,146** | 当前生产线；Erigon 借鉴一轮见 `docs/review/`：HPH 缓存 + 标签合入（41m49s / 307,369，噪声内），Keccak 具体类型/fastkeccak/Reset clear() 三项撤回 |
| **perf/instr-count（ic6，128w/6r/gc300）** | **41m34s** | **305,426** | 解释器减指令数一轮，见 §5.32；1M 切片 −20% CPU |
| **生产配置（main 合并后，128w/6r/gc300）** | **49m27s** | **373,462** | 合并后 main 实测；rebased2 48m39s / 371,444；tier-1 3/3、tier-2、T1、t3 各 1/1 全部通过 |
