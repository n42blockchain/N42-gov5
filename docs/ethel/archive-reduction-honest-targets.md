...
| Warm CS (7 days) | **2.44 GB** ✓ (acctcs 825 MB + storcs 1.64 GB, late-chain density) |
| Blocks (full chain data) | ~150 GB |
| **Full archive** | **~317 GB** |
| **State-only archive (no blocks)** | **169.13 GB** ✓ |
...
- State-only: **5.59× smaller** (945 → 169.13 GB measured)
- Fast mode: **31× smaller**

**CS warm tier impact** (commit `c61a5726`/`62436e98`):
- Original acctcs + storcs: **397 GB**
- After 7-day (50,400 blocks) warm prune: **2.44 GB** (99.4% reduction)
  - acctcs 825 MB + storcs 1.64 GB
- Tool: `cmd/n42-cs-prune` (build) + `cmd/n42-cs-prune-verify` (round-trip)
- 50,400-block prune in **64 sec wall**; 10,000-sample verify 100% match
- Late-chain density makes per-block CS ~3× the average; the early
  dry-run estimate of 820 MB extrapolated from src-total-size and was
  off by 3×. Measured wins remain dominant.
