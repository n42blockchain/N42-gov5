...
1. 等 w256（SMT 判决）、tier-1 A/B（同配置只换 binary）、r6 干净重跑三个排队运行出齐；
2. 从 w256 的 CPU 占用（约 129 线程，远低于 256）看，**6 个 reader 喂不饱 256 个 worker**，
   下一个单变量是 readers 8 / 12；
3. `perf stat` 量 IPC（需要 `kernel.perf_event_paranoid=1`）。
