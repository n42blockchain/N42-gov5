...
### 5.21 定向复现 0→13M × 4：全部通过

tier-1-diag、256w / 6r / gc300、`--end 13000000`（失败块 12,854,611 之后）：

| 轮 | wall | CPU-sec | 结果 |
|---|---:|---:|---|
| 1 | 11m47s | 82,507 | failed=0 |
| 2 | 12m00s | 82,215 | failed=0 |
| 3 | 13m08s | 79,068 | failed=0 |
| 4 | 11m48s | 82,245 | failed=0 |

256w/gc300 下 tier-1 至此 **6 过 1 败**（全量 2/3 + 0→13M 4/4），诊断从未触发。
...
### 5.23 推荐配置可靠性：128w / 6r / gc300 tier-1，3/3 通过，方差 < 1%

| 轮 | wall | CPU-sec | CPU% | MaxRSS | failed | 诊断 |
|---|---:|---:|---:|---:|---|---|
| r1 | 54m10s | 394,705 | 12143% | 16.1 GiB | 0 | 未触发 |
| r2 | 54m35s | 398,336 | 12163% | 15.9 GiB | 0 | 未触发 |
| r3 | 54m30s | 397,714 | 12161% | 16.0 GiB | 0 | 未触发 |

三轮均经 sar 核对独占（整机 107–132 忙碌线程 ≈ 本进程 121）。墙钟极差 25 秒（0.8%），
CPU-sec 极差 0.9%。**这是生产推荐配置**：`scripts/witness-full.sh` 默认改为
128w / 6r / GOGC 300 / 56 GiB。

相对本文起点（08-24 基线 2h27m10s / 456,881 CPU-s）：墙钟 **−63%**，CPU-sec **−13%**，
两者同时。

### 5.24 tier-2（M1 journal 去装箱）A/B：分配器锁竞争假设得到验证

1M 密集切片，freezer，gc300，交替 binary，独占（sar 152–184 ≈ 本进程）：

| 行 | wall | blk/s | CPU-sec | 忙碌线程 | MaxRSS |
|---|---:|---:|---:|---:|---:|
| tier-1 @256w r1 / r2 | 5:50.0 / 5:48.4 | 2857 / 2870 | 60,553 / 60,710 | 173 / 174 | 14.7 / 13.7 GiB |
| **tier-2 @256w r1 / r2** | **5:22.8 / 5:21.8** | **3098 / 3107** | 62,094 / 61,906 | **192 / 192** | 14.2 / 14.0 |
| tier-1 @128w | 6:08.2 | 2716 | 46,482 | 126 | 10.5 |
| **tier-2 @128w** | **5:52.9** | **2834** | **45,572** | 129 | 10.4 |

- 256w：墙钟 **−7.8%**，忙碌线程 173 → 192——正是 5.22 预测的方向：少了每条 journal
  一次分配，`mcentral` 锁上的排队变短，空转 worker 回来干活。CPU-sec +2.3% 是更多
  SMT 兄弟线程忙碌的记账，不是真实开销。
- 128w：墙钟 **−4.2%**，CPU-sec **−2.0%**（这里没有锁竞争，是纯粹的分配/GC 收益）。
- 两轮 256w 结果之间差 1 秒，重复性好。

**tier-2 采纳。** 全量确认（128w 生产配置 + 256w）排队中。

### 5.25 tier-2 全量确认：两个配置都创纪录

binary `witness-replay.tier2`（`2df12dce`，含诊断），独占（sar 核对）：

| 配置 | binary | wall | CPU-sec | CPU% | MaxRSS | failed |
|---|---|---:|---:|---:|---:|---|
| 128w / 6r / gc300 | tier-1 | 54m10s | 394,705 | 12143% | 16.1 GiB | 0 |
| **128w / 6r / gc300** | **tier-2** | **50m32s** | **384,638** | 12686% | 16.2 GiB | 0 |
| 256w / 6r / gc300 | tier-1 | 51m13s | 487,214 | 15854% | 23.3 GiB | 0 |
| **256w / 6r / gc300** | **tier-2** | **45m59s** | 494,771 | 17931% | 25.2 GiB | 0 |

- 128w：墙钟 −6.7%、CPU-sec −2.6%——**双纪录**，生产配置更新为 tier-2。
- 256w：墙钟 −10.2%（忙碌线程 159 → 179，分配器锁竞争继续缓解），CPU-sec +1.6%（SMT 记账）。
  **首次全量跌破 46 分钟**。256w/gc300 至此 tier-1/2 合计 4 过 1 败，诊断仍未触发。

相对起点（2h27m10s / 456,881）：128w **墙钟 −66%、CPU-sec −16%**；256w 墙钟 **−69%**。

### 5.26 T1（单表 storage + epoch）：输出逐字节一致，切片 +1.4–1.6%

**输出模式护栏**：tier-2 与 T1 各带 `--output --receipts` 重放 `0..4,000,000`
（128w/6r/gc300），各写出 9 个 cdat/cidx 共 3.5 GB：**sha256 全部一致**。
写集（acctcs / storcs / wipes / receipts）逐字节不变。

**1M 密集切片 A/B**（gc300，交替，独占）：

| 行 | wall | blk/s | CPU-sec | 忙碌线程 | MaxRSS |
|---|---:|---:|---:|---:|---:|
| tier-2 @256w r1 / r2 | 5:21.9 / 5:22.0 | 3107 / 3106 | 61,748 / 61,338 | 192 / 190 | 14.1 / 15.1 GiB |
| **T1 @256w r1 / r2** | **5:16.3 / 5:18.8** | 3161 / 3136 | 61,194 / 60,996 | 193 / 191 | 18.6 / 18.2 |
| tier-2 @128w | 5:48.9 | 2867 | 45,035 | 129 | 10.5 |
| **T1 @128w** | **5:43.4** | 2912 | 44,318 | 129 | 12.8 |

256w 墙钟 −1.4%、CPU-sec −0.7%；128w 墙钟 −1.6%、CPU-sec −1.6%。三对都是正向，
...
### 5.28 t3（M3 余额栈临时值 + M5 LOG arena）：切片 256w −3.0%，全量 −0.5%

注意：第一次 A/B 的三行因链脚本继承了编译 shell 的 `GOMAXPROCS=8` 而无效
（每行 795% CPU、25 分钟），已作废并重跑；`full-run.sh` 与链脚本现在开头 `unset GOMAXPROCS GOCACHE`。

1M 密集切片（gc300，交替，独占）：

| 行 | wall | blk/s | CPU-sec | 忙碌线程 | MaxRSS |
|---|---:|---:|---:|---:|---:|
| T1 @256w r1 / r2 | 5:20.8 / 5:18.9 | 3128 / 3148 | 61,345 / 61,104 | 191 / 192 | 17.0 / 17.3 GiB |
| **t3 @256w r1 / r2** | **5:09.8 / 5:09.6** | 3242 / 3244 | 61,100 / 60,948 | 197 / 197 | 23.1 / 23.0 |
| T1 @128w | 5:43.7 | 2916 | 44,214 | 129 | 12.5 |
| **t3 @128w** | **5:40.5** | 2943 | 43,843 | 129 | 14.9 |

全量 gate，128w / 6r / gc300，独占（sar 108–132 ≈ 126）：

```text
run_id       full-geth-w128-t3-gc300-20260827
wall         49m48s            （T1 50m02s，−0.5%）
user+sys     376,118 s         （T1 378,758，−0.7%）
MaxRSS       22.2 GiB          （T1 19.1）
failed       0   诊断未触发
```

256w / 6r / gc300 同 binary：46m08s / 476,075 CPU-s / MaxRSS 37.1 GiB，failed=0
（T1：45m56s / 482,741 / 29.0 GiB）——墙钟持平（+0.4%，噪声内），CPU −1.4%，RSS +8 GiB。
切片上的 −3% 没有在全量上兑现。256w/gc300 累计 11 次 1 败。
...
### 5.33 基本块级 gas/栈预检：负结果（perf/block-precheck，未合并）

evmone "advanced" 的思路：块入口一次性收整块常量 gas、一次性检查栈需求，块内 op 不再各自
比较/扣费/读 meta 表。实现在分支 `perf/block-precheck`（`74452740`，全部单测通过）。

**必须遵守的约束（这轮才发现）**：witness 是**顺序访问流**（`WitnessReplayReader.pos`
逐条消费），重放必须按原始执行完全相同的顺序做完全相同的状态读取——**包括注定失败的帧内部**。
第一版预检在块入口/动态 gas 处提前 OOG，跳过了一次 SLOAD，reader 游标错位，块 24,999,104
tx 77 的发送者账户解码成 "proto: cannot parse invalid wire-format"。8 个跨分叉 sanity 窗口
在 gate 之前抓到。改成"预检只在预测成功时生效，任何预测失败退回逐 op 精确计费"，并且所有
动态 gas op 都加回"块内未花的 rest"（观察到的剩余 gas 与 OOG 点精确一致）。单测比对结果、
剩余 gas 与逐 op 执行流（300 随机程序 × 121 gas 值）。

**结果**（128w/6r/gc300）：

| | 1M 切片 CPU-s | 全量 |
|---|---|---|
| main（ic6 + pool 开关，`main-20260827b`） | 34,271 / 4m31s；34,196 / 4m29s | 41m34s / 305,426 |
| bp2（预检） | 35,408 / 4m36s（+3.3%） | **44m41s / 318,062（+4.1% CPU，+7.5% 墙）**，failed=0，RSS 30.3 GiB |
| bp3（去掉两处冗余：内存费算两次、逐 op 测试钩子） | 35,469 / 4m41s（+3.7%） | — |

**为什么不赚**（200k 窗口 profile，main 6962 s vs bp3 7143 s 总样本）：`Run` flat 1767 → 1404 s
（每 op 省下的比较/扣费/meta 读 = 363 s），但换来 `tryEnter` 142 + `index`（rank+popcount）113 +
`rest` 125 + `execTable` 调用 66 + `expandMemoryInline` 27 ≈ 470 s。基本块平均只有 ~10 条指令
