...
## 13. zstd 静态段实测:抵达设计带(2026-06-11,segexport)

前缀差分平铺流 + zstd,对 2M 数据全表实测:

| 表 | MDBX | raw 流 | zstd-max 段 | 倍率 | 哈希置零对照 |
|---|---|---|---|---|---|
| AccNode | 2.1 GB | 863 MB | **539 MB** | 3.9x | **31.6 MB**(哈希=全部熵) |
| AccChg | 709 MB | 215 MB | 137 MB | 5.2x | 137 MB(无哈希,纯结构) |
| StorChg | 1.1 GB | 151 MB | **72 MB** | **15x** | 72 MB |
| LeafA | 1.5 GB | 224 MB | 147 MB | 10x | — |
| LeafS | 649 MB | 101 MB | 63 MB | 10x | — |
| **合计** | **6.5 GB** | 1.5 GB | **957 MB** | **6.8x** | 449 MB |

**熵切分(理论验证)**:957MB = **508MB 不可压哈希熵 + 449MB 可压结构**。
AccNode 哈希置零后只剩 31.6MB —— 节点段的体积几乎纯由变更子哈希构成,
与 §5 的熵下界论证逐字吻合;且哈希原始载荷 780MB 被 zstd 去重到 ~508MB
(FULL 记录每 8 epoch 重复未变子哈希,窗口内可去重 —— F 调大还有余量)。

**摊薄成本:60B/叶写(全套)/ 34B/叶写(仅节点段)** —— 距设计估算 26B
仅 1.3-2.3x,已抵达设计带。

### 最终投影

| 链 | 全套段文件 | chg-free* + historicalstate 换叶史 |
|---|---|---|
| N42 13M(T_w≈1 亿) | **~6 GB** | ~3.4 GB |
| ETH 25M(T_w≈5-7B) | **~300-420 GB** | **~170-240 GB** ✓ 设计带(130-250GB)命中 |

\* chg-free 变体:变更窗口从 changesets(归档本有)按需推导 —— E_d 随
