...
- `replay_stats.currentBlock` records source progress and must equal H. The
  exported `checkpoint.json` records the target canonical head; with gap-fill
  enabled its number is larger than H by the synthetic-block count.
