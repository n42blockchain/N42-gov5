...
function BuildArgs($i){
  ...
  return @('--chain','mainnet_qmdb','--profile','n42',
...
```powershell
# Stop only this build's nodes (do NOT kill unrelated n42 processes).
Get-Process n42 -ErrorAction SilentlyContinue |
  Where-Object { $_.Path -eq 'C:\N42\N42-gov5\build\bin\n42.exe' } |
  Stop-Process -Force
```

Nodes flush + commit on SIGINT/stop; the data dir is reusable on next start
(resume from the persisted head).
