...
4. `Phase 4` 已固定 `make interop-smoke`、`make soak-smoke` 和 `make release-check`，覆盖 RPC / Blockscout / Hive `engine-auth` / EEST collect-only / EEST 结果目录审计 / 重启循环 / 短压测；其中 `interop-smoke` 使用临时 `n42 --ethdev` 节点，`maturity-baseline` 仍只跑包级 gate。
...
4. `make release-check` 已把 `maturity-baseline + eest-audit + ops-smoke + interop-smoke + soak-smoke` 收成单一发布 gate。
