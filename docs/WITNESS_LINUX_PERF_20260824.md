...
- Inputs are the existing columnar `headerc/bodyc`, witness and senders tables
  under `/data/blockchain/witness`; bytecode comes from the complete
  content-addressed Code table in `/data/blockchain/code-mdbx`.
...
2. Existing bodyc data is repaired on read only when needed. For a block with a
   zero-V EIP-7702 authorization, the reader compares the reconstructed
   transaction root with the canonical `headerc` `TxHash`; on mismatch it tries
   the legacy 27/28 value and accepts a repair only when the canonical root
   matches. Ethereum authorization validation rules are not changed.
...
Use 128/300/64 for the shortest full run. Use 112/300/56 for almost the same
wall time with lower CPU/RAM, or 128/100/48 when RAM must be yielded.
...
Run only after other host workloads and the N42 fleet are stopped:
...
/data/blockchain/bin/witness-replay \
  ...
  --workers 128 --gogc 300 --mem-limit-gb 64
```
