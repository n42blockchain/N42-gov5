...
// ClearBucket on a handled table must also drop the overlay's pending writes for
// it, else the committed table is wiped while the overlay still shadows stale
// rows. Today this is only reached by the first-window legacy full-rebuild (empty
// committed + empty overlay → no-op), but intercepting it hardens against any
// future ClearBucket-against-a-populated-overlay misuse.
func (t *stateOverlayRwTx) ClearBucket(table string) error {
	if t.o.Handles(table) {
		t.o.tree(table).Clear()
	}
	return t.RwTx.ClearBucket(table)
}
...
func (t *stateOverlayRwTx) CursorDupSort(table string) (kv.CursorDupSort, error) {
	...
	if table == modules.HashedStorage {
		mc.tolerateBaseSeekEOF = true // AutoDupSort flat-Seek EOF dance (see mergedCursor.Seek)
...
func (t *stateOverlayTx) CursorDupSort(table string) (kv.CursorDupSort, error) {
	...
	if table == modules.HashedStorage {
		...
		// and it is the only AutoDupSort table whose flat-Seek can surface an EOF
		// error that must be tolerated (see mergedCursor.Seek).
...
// NextDup returns the account key as c.curAddr, whose backing array the NEXT
...
func (c *stateDupCursor) NextNoDup() ([]byte, []byte, error) {
	return nil, nil, errUnsupportedDup("NextNoDup")
}
func (c *stateDupCursor) PrevDup() ([]byte, []byte, error) {
	return nil, nil, errUnsupportedDup("PrevDup")
}
func (c *stateDupCursor) PrevNoDup() ([]byte, []byte, error) {
	return nil, nil, errUnsupportedDup("PrevNoDup")
}
func (c *stateDupCursor) LastDup() ([]byte, error) { return nil, errUnsupportedDup("LastDup") }
func (c *stateDupCursor) CountDuplicates() (uint64, error) {
	return 0, errUnsupportedDup("CountDuplicates")
}
