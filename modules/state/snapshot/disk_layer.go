...
//
// DiskLayer: bottom layer of the snapshot tree.
// Backed by a layered.ShardedCache plus optional SnapshotAccount/
// SnapshotStorage flat tables. When genReady is set reads consult
// the cache first then fall back to the flat tables, eliminating the
// full state reader fallback for snapshot-covered keys. SetDB wires
// the RW database after persistent mode is enabled.
