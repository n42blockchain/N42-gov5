...
func NewAPI(reg *Registry, certs *CertStore, wins *WindowManager) *API {
	return &API{reg: reg, certs: certs, wins: wins}
}
...
func (a *API) GetCertificates(blockHash types.Hash) []certJSON {
	return encodeCerts(a.certs.Get(blockHash), a.reg.Count())
}
