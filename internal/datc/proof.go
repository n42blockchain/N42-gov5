...
func (q *querier) proofPath(domain, fullNib []byte, n uint64) ([][]byte, error) {
	...
	for {
		...
		if !usable {
			...
			nodes = append(nodes[:first], subtreeProofOrder(nodes[first:])...)
...
// subtreeProofOrder puts the path nodes mptNodeRLP collected into EIP-1186
// order. mptNodeRLP appends a node after its on-path child (it needs the
// child's hash first), so the list comes out deepest-first; a proof is
// root-first. A node shorter than 32 bytes below the subtree root is embedded
// in its parent's RLP and is not a proof entry of its own. The subtree root
// is always listed: its parent branch references it by hash (or, at the top,
// the header does).
func subtreeProofOrder(seg [][]byte) [][]byte {
	out := make([][]byte, 0, len(seg))
	for i := len(seg) - 1; i >= 0; i-- {
		if len(out) > 0 && len(seg[i]) < 32 {
			continue
		}
...
func walkProof(root types.Hash, nodes [][]byte, keyNib []byte) ([]byte, error) {
	// EIP-1186: the proof is the path, root first. Each node must be the one
	// its predecessor references by hash (embedded children are part of their
	// parent's RLP), and nothing may follow the node that decides the key.
	// Order matters to every client verifier; checking only that the nodes
	// hash-link as a SET let a deepest-first tail pass here for months.
	if len(nodes) == 0 {
		return nil, nil // empty trie / absence at the very top
	}
	if keccak(nodes[0]) != root {
		return nil, fmt.Errorf("root node %x not first in proof", root[:8])
	}
	idx := 0
	cur := nodes[0]
	done := func(v []byte) ([]byte, error) {
		if idx != len(nodes)-1 {
			return nil, fmt.Errorf("%d proof node(s) after the one that decides the key", len(nodes)-1-idx)
		}
		return v, nil
	}
	// follow resolves a child reference: an embedded node is decoded in place,
	// a hash must be the next listed node.
	follow := func(ref []byte, where string) error {
		switch {
		case len(ref) == 32:
			idx++
			if idx >= len(nodes) {
				return fmt.Errorf("missing node %s", where)
			}
			if !bytes.Equal(keccak(nodes[idx]).Bytes(), ref) {
				return fmt.Errorf("proof node %d is not the node referenced %s", idx, where)
			}
			cur = nodes[idx]
		case len(ref) > 0:
			cur = ref // embedded node: its own RLP, carried by the parent
		default:
			return fmt.Errorf("malformed child ref %s", where)
		}
		return nil
	}
	depth := 0
	for {
		...
		case 17:
			if depth == len(keyNib) {
				return done(items[16])
			}
			ref := items[keyNib[depth]]
			if len(ref) == 0 {
				return done(nil) // absent child: exclusion
			}
			depth++
			if err := follow(ref, fmt.Sprintf("at depth %d", depth)); err != nil {
				return nil, err
			}
		case 2:
			...
			if leaf {
				...
				return done(nil) // different leaf occupies the slot: exclusion
			}
			if len(keyNib)-depth < len(pathNib) || !bytes.Equal(keyNib[depth:depth+len(pathNib)], pathNib) {
				return done(nil) // extension diverges: exclusion
			}
			depth += len(pathNib)
			if err := follow(items[1], fmt.Sprintf("below the extension at depth %d", depth)); err != nil {
				return nil, err
			}
		default:
