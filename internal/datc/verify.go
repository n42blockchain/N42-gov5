...
func loadQuerierCache(tx kv.Tx, out string, foldOverride int, frameCache int) (*querier, uint64, error) {
	...
	q := &querier{tx: tx, sched: sched, accFold: int(accDepth), stoFold: int(stoDepth), stoRootPerBlock: cad == 1, crossCheck: proofCrossCheck}
...
type querier struct {
	...
	// crossCheck: proofPath re-hashes every folded subtree with the second
	// builder (proofCrossCheck at creation).
	crossCheck bool
