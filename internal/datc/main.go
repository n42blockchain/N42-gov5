...
// Main is the n42-datc command: build, verify, prove and maintain a DATC
// archive (cmd/n42-datc is a thin wrapper, so the reader can also be linked
// into a node — see archive.go).
func Main() {
	if len(os.Args) < 2 {
	...
	if os.Args[1] == "verify-ns" {
		runVerifyNS(os.Args[2:])
		return
	}
