...
// segHold keeps the frame index of every segment of one archive loaded: it
// owns a reference to each. Parsing a 16 KiB-frame node segment's index takes
// tens of milliseconds, which a long-lived reader pays at start-up instead of
// inside the first proofs that touch each segment.
type segHold struct {
	entries []*segFileEntry
}

// holdSegFiles loads and holds every segment under outDir.
func holdSegFiles(outDir string, workers int) (*segHold, error) {
...
// preloadSegFiles holds every segment under outDir for the life of the
// process — what a command (bench, derive-ns) wants. A library user holds
// them through holdSegFiles and releases them itself (Archive).
func preloadSegFiles(outDir string, workers int) (int, error) {
	h, err := holdSegFiles(outDir, workers)
	if err != nil {
		return 0, err
	}
	return len(h.entries), nil // never released: process lifetime
}
