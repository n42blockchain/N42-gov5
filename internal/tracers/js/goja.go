...
func init() {
	// Keep JS eval available even if embedded built-in tracers fail to load.
	tracers.DefaultDirectory.RegisterJSEval(newJsTracer)
	loaded, err := jsassets.Load()
	if err != nil {
		return
	}
	assetTracers = loaded
...
func fromBuf(vm *goja.Runtime, bufType goja.Value, buf goja.Value, allowString bool) ([]byte, error) {
	...
	case "String":
		...
		return common.FromHex2Bytes(obj.String()), nil
