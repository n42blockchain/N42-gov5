...
func TestSD_Forward_SameBlock_SSTORE_then_SD_FirstWins(t *testing.T) {
	...
	require.NoError(t, w.WriteAccountStorage(addr, 0, &slot, orig, val))
...
func TestSD_Forward_RecreateSameContext(t *testing.T) {
	...
	require.NoError(t, w.WriteAccountStorage(addr, 0, &newSlot, zero, val))
...
func TestSD_Forward_RecreateNextBlock(t *testing.T) {
	...
	zero := uint256.NewInt(0)
	require.NoError(t, w2.WriteAccountStorage(addr, 0, &newSlot, zero, val))
...
func TestSD_Forward_RecreateLaterBlock_DifferentSlots(t *testing.T) {
	...
	for _, s := range newSlots {
		...
		require.NoError(t, w2.WriteAccountStorage(addr, 0, &s.slot, zero, v))
	}
...
func TestSD_Forward_AsyncBuffer_CrossInterval(t *testing.T) {
	...
	require.NoError(t, w1.WriteAccountStorage(addr, 0, &slot, orig, val))
...
func TestSD_Forward_SSTOREToZero_SingleSlotDelete(t *testing.T) {
	...
	require.NoError(t, w.WriteAccountStorage(addr, 0, &zeroSlot, orig, zero))
...
func TestSD_Reorg_AcrossSelfDestruct_Depth10(t *testing.T) {
	...
	ops := []blockOps{
		...
		func(w *state.PlainStateWriter, cur *account.StateAccount) *account.StateAccount {
			slot := sdHash(0x01)
			require.NoError(t, w.WriteAccountStorage(addr, 0, &slot, uint256.NewInt(0x01), uint256.NewInt(0x11)))
			return cur
		},
		// block 2: SSTORE addr.slot03 := 0x33 (new slot)
		func(w *state.PlainStateWriter, cur *account.StateAccount) *account.StateAccount {
			slot := sdHash(0x03)
			require.NoError(t, w.WriteAccountStorage(addr, 0, &slot, uint256.NewInt(0), uint256.NewInt(0x33)))
			return cur
		},
		// block 3: nonce bump
		func(w *state.PlainStateWriter, cur *account.StateAccount) *account.StateAccount {
		...
		func(w *state.PlainStateWriter, cur *account.StateAccount) *account.StateAccount {
			slot := sdHash(0xFF)
			require.NoError(t, w.WriteAccountStorage(other, 0, &slot, uint256.NewInt(0xFF), uint256.NewInt(0xEE)))
			return cur
		},
		...
		func(w *state.PlainStateWriter, cur *account.StateAccount) *account.StateAccount {
			...
			require.NoError(t, w.CreateContract(addr))
			slot := sdHash(0x07)
			require.NoError(t, w.WriteAccountStorage(addr, 0, &slot, uint256.NewInt(0), uint256.NewInt(0x77)))
			return newAcc
		},
		// block 8: SSTORE addr.slot08
		func(w *state.PlainStateWriter, cur *account.StateAccount) *account.StateAccount {
			slot := sdHash(0x08)
			require.NoError(t, w.WriteAccountStorage(addr, 0, &slot, uint256.NewInt(0), uint256.NewInt(0x88)))
			return cur
		},
		...
		func(w *state.PlainStateWriter, cur *account.StateAccount) *account.StateAccount {
			slot := sdHash(0x07)
			require.NoError(t, w.WriteAccountStorage(addr, 0, &slot, uint256.NewInt(0x77), uint256.NewInt(0)))
			return cur
		},
	}
