...
var (
	...
	zeroU64            = uint64(0)
	defaultExtra       = make([]byte, 32) // 32 zero bytes (ETH standard default)
)
	...
	return e.dstDB.Update(ctx, func(dstTx kv.RwTx) error {
		...
		return e.srcDB.View(ctx, func(srcTx kv.Tx) error {
			for num := from; num <= to; num++ {
				...
				if e.cfg.FillGaps && prevTime > 0 {
					...
					for _, gapTime := range gapTimes {
						...
						gapHeader := &block.Header{
							...
							WithdrawalsHash:  &emptyRewardsHash,
							BlobGasUsed:      &zeroU64,
							ExcessBlobGas:    &zeroU64,
							ParentBeaconRoot: &gapParentCEHash,
				...
				// Build ETH-standard header with all 21 fields.
				newHeader := &block.Header{
					...
					BaseFee:          srcHeader.BaseFee,
					WithdrawalsHash:  &rewardsHash,
					BlobGasUsed:      &zeroU64,
					ExcessBlobGas:    &zeroU64,
					ParentBeaconRoot: &parentCEHash,
					RequestsHash:     &emptyRequestsHash,
				}
