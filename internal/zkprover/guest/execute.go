...
//
// Execute: zkVM entry point that iterates over a block's
// transactions, calls ApplyTx for each one and emits the public
// inputs (pre-state root, post-state root, block hash, gas used)
// consumed by the prover to produce the succinct execution proof.
