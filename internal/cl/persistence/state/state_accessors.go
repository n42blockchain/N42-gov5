...
// Ported from erigon cl/persistence/state/state_accessors.go (imports rewritten
// erigon/cl/* → internal/cl/*, db/kv → lib/kv, db/snapshotsync → the N42
// DB-fallback shim at internal/cl/depshim/snapshotsync). Part of the Caplin
// merge (#31): the DB read/write accessors antiquary + historical_states_reader
// build on. With the shim's empty CaplinStateView, GetValFnTxAndSnapshot always
// takes the tx.GetOne DB path.
...
// ReadNextSyncCommittee reads the next sync committee from the database.
...
// ReadCurrentSyncCommittee reads the current sync committee from the database.
