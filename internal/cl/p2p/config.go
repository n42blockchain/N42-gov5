...
// The explicit /mplex muxer was dropped: N42 standardized on go-libp2p
// v0.48.0 whose DefaultMuxers (yamux) supersede mplex (#34 libp2p cleanup).
...
func buildOptions(cfg *P2PConfig, privateKey *ecdsa.PrivateKey) ([]libp2p.Option, error) {
	...
	options := []libp2p.Option{
		...
		libp2p.UserAgent("n42/caplin"),
		libp2p.Transport(tcp.NewTCPTransport),
		libp2p.DefaultMuxers, // yamux (mplex dropped — superseded in go-libp2p v0.48.0)
		libp2p.Ping(false),
	}
	if cfg.EnableUPnP {
