...
//
// Ported from erigon cl/sentinel. Adaptations (#34): import paths rewritten to
// N42 in-repo equivalents; prysmaticlabs/go-bitfield in Identity() replaced
// with raw []byte (attnets 8B, syncnets 1B) consistent with cl/p2p.
...
import (
	...
	"github.com/n42blockchain/N42/internal/cl/sentinel/handlers"
...
type Sentinel struct {
	...
	// calls to work around a data race in libp2p's memoryAddrBook.
...
func New(
	...
	// stdlib ServeMux (chi dropped — the router only needs to dispatch "/" to
	// the in-process ReqResp handler, which httpreqresp.Do calls via GET).
	mux := http.NewServeMux()
	mux.Handle("/", httpreqresp.NewRequestHandler(s.p2p.Host()))
	s.httpApi = mux
...
func (s *Sentinel) Start() (*enode.LocalNode, error) {
	...
	go s.listenForPeers()
	go s.proactiveSubnetPeerSearch() // no-op in block-only mode
...
func (s *Sentinel) Identity() (pid, enrStr string, p2pAddresses, discoveryAddresses []string, metadata *cltypes.Metadata) {
	...
	if nodeIP == nil {
		s.logger.Warn("[Sentinel] Discovery node has nil IP address, skipping discovery address advertisement.")
	} else {
		protocol := "ip4"
		if nodeIP.To4() == nil {
			protocol = "ip6"
	...
	// attnets = Bitvector64 (8 bytes), syncnets = Bitvector8/4 (1 byte).
	subnetField := make([]byte, 8)
	syncnetField := make([]byte, 1)
