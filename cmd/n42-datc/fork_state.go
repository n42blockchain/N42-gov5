...
func runForkState(args []string) {
	...
	sdb, err := mdbxkv.NewMDBX(logger).Path(*src).Label(kv.ChainDB).
		...
		WithTableCfg(datcTableCfg).Open(context.Background())
	if err != nil {
		die("open src: %v", err)
	}
	defer sdb.Close()
...
// datcTableCfg registers the DATC prototype tables on top of the chaindata set.
// fork-state needs it on BOTH ends: the source has DatcMeta physically but the
// kv layer will not open an unregistered table, and the destination is a fresh
// database where DatcMeta has to be CREATED. Without it the four state tables
// copy fine and the run dies at the very end on
// "meta: table: DatcMeta, mdbx_cursor_open: input/output error" — 22 minutes in.
// (Not a merge regression: fork-state is Pipeline B's first step and Pipeline B
// had never been run, which is why the node records still stop at 15.22M.)
func datcTableCfg(_ kv.TableCfg) kv.TableCfg {
	d := kv.TableCfg{}
