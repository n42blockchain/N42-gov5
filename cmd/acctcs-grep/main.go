// acctcs-grep: grep a hex byte pattern across all acctcs freezer blobs.
package main
...
func main() {
	...
	tbl, err := freezer.NewFreezerTableReadOnly(*freezerDir, "acctcs", "c")
	if err != nil {
	...
	fmt.Printf("scanning n42 acctcs [%d, %d] for pattern=%x (%dB)\n",
